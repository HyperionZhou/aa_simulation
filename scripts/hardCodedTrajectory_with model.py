...
def modify_state_curve(state, move_param):
    ...
    return np.array([dx, theta, ddx, dtheta, vel, 0.7])
...
def modify_state_straight(state, move_param):
    ...
    # print("new dyaw: ", dyaw)
    vel = np.sqrt(np.square(x_dot) + np.square(y_dot))
    vel_dir = np.arctan2(y_dot, x_dot)
    vel_dir = _normalize_angle(vel_dir - _normalize_angle(target_dir))
...
def rollout(env, agent, way_point=[], animated=False, speedup=1,
            ...
            isCurve=False, move_param=[], model=None):
    # state2 = np.zeros(6)
    while _check_point(state, way_point):
        ...
        # print("state's o: ", o)
        #
        action, agent_info = agent.get_action(o)
        # o2 = modify_state_straight(state2, [0, 0, 0])
        # print("state2's o: ", o2)
        # print("correct dx dy: ", state[3], state[4])
        # action2, _ = agent.get_action(o2)

        # print("action: ", action)
        # print("action2: ", action2)
        # next_o, r, d, env_info = env.step(a)
        state = model.state_transition(state, action, _dt)
        # state2 = model.state_transition(state2, action2, _dt)


        if animated:
            render(renderer, state, action)
            #env.render()
            timestep = 0.05
...
def move(env, policy, args, way_point, renderer,\
         state, isCurve, move_param, model):
    final_state = rollout(env, policy, way_point=way_point,
                        ...
                        move_param=move_param, model=model)
    return final_state
...
def main():
    stream = open('aa_simulation/envs/model_params.yaml', 'r')
    _params = yaml.load(stream)
    _model = VehicleModel(_params)

    args = parse_arguments()

    data_curve = joblib.load("data/local/preparedPolicy/curve.pkl")
    policy_curve = data_curve['policy']
    env_curve = data_curve['env']

    data_straight = joblib.load("data/local/preparedPolicy/straight.pkl")
    ...
    state = [3, 0, np.deg2rad(90), -0.05, 0.05, 0]
    ...
    for i in range(400):
        ...
        state = move(env_straight, policy_straight, args,\
                way_points[point], renderer, state,\
                False, straight_params[1], _model)
        point += 1
